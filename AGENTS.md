...
- The `chat.params` hook tags opencode's active agent (`default`, `compaction`, `title`, etc.) into provider options as `opencodeAgent`, and also injects `input.sessionID` as `opencodeSessionID`. Write to `output.options` at the top level. opencode wraps that bag under the provider id later. Do not pre-nest under `output.options[providerID]`, or the model sees `providerOptions[id][id]`.
- Session affinity for Claude CLI process isolation uses `resolveSessionAffinity()` (exported from `src/claude-code-language-model.ts`). Priority: (1) `x-session-affinity` request header — primary, set by opencode for third-party providers; (2) `opencodeSessionID` from providerOptions — fallback injected by the `chat.params` hook, covers provider-switch mid-session and title synthesis paths where the header is absent; (3) `"default"`. The session key in `session-manager.ts` is `${cwd}::${modelId}::${scope}::${affinity}`, so a unique affinity token guarantees separate CLI processes for concurrent opencode sessions. Tests live in `test-session-affinity.ts`.
